/**
 * Originkit Tornado — WebGL Particle Vortex Engine (Vanilla JS Edition)
 * Author: Antigravity AI
 * Pure JavaScript equivalent of the React Three.js Tornado/Vortex component.
 */
...
(function () {
  ...
  function createVortex(canvas, container, cfgRef) {
    ...
    function settle(dt) {
      ...
      for (let i = 0; i < dotCount; i++) {
        ...
        const restSq = Math.pow(dotShift[at], 2) + Math.pow(dotShift[at + 1], 2) + Math.pow(dotShift[at + 2], 2);
        const velSq = Math.pow(dotVel[at], 2) + Math.pow(dotVel[at + 1], 2) + Math.pow(dotVel[at + 2], 2);
    ...
    function driveComet(comet, now, dt, entrance) {
      ...
      for (let i = 0; i < tailLen; i++) {
        ...
        const along = Math.pow(1 - i / tailLen, 2);
      ...
      if (dotMesh && dotCount > 0) {
        ...
        for (let i = 0; i < dotCount; i++) {
          ...
          const beat =
            ...
            cfg.dotFlicker +
            cfg.dotFlicker *
              Math.pow(0.08 + 0.92 * Math.max(0, Math.sin(t * dot.flickerRate + dot.pulse)), 2.5);
  ...
  // Export globally
  window.createVortex = createVortex;
})();
